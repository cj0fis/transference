@tool
class_name PhysicsDungeonGenerator extends Node2D

@export_tool_button("generate rects") var a1 = generate_rects
@export_tool_button("resolve overlaps") var a2 = resolve_overlaps
@export var params: DungeonParameters
@export var rect_count: int = 20
@export var rect_size: Vector2 = Vector2(10,25)
...
func _draw() -> void:
	...
	for node in spatial_graph.discarded_nodes:
		draw_rect(node.rect, Color(0,0,0), false, -10.0)
	#draw rooms
	for node in spatial_graph.nodes:
		draw_rect(node.rect, Color(0.6,0.6,0.6), false, -10.0)
		...
		draw_rect(node.room.rect, color, true)
	for line in spatial_graph.hallway_lines:
			draw_line(line.position, line.size, Color(0.4,0.4,0.4), 1.0)
			
func get_rand_point_in_circle(radius: int) -> Vector2i:
	var t = 2 * PI * randf()
	var u = randf() + randf()
	var r = 2-u if u > 1 else u
	return Vector2i(int(radius * r * cos(t)),int(radius * r * sin(t)))
...
func generate_rects() -> void:
	...
	rects.clear()
	var center: Vector2i = world_size / 2
	for i in range(rect_count):
		var pos = get_rand_point_in_circle(spawn_radius) + center
		var size = Vector2i(randi_range(rect_size.x,rect_size.y), randi_range(rect_size.x,rect_size.y))
		rects.append(Rect2i(pos - size/2, size))
		queue_redraw()
		await get_tree().create_timer(0.01).timeout
	await resolve_overlaps()
	spatial_graph = SpatialGraph.new(rects, params)
	rects.clear()
	...
	for iter in range(max_iterations):
		...
		for i in range(rects.size()):
			for j in range(i+1, rects.size()):
				...
				if r1.intersects(r2):
					moved = true
					var diff = r1.get_center() - r2.get_center()
					var dir: Vector2i

					if abs(diff.x) > abs(diff.y):
						dir = Vector2i(sign(diff.x), 0) # push horizontally
					elif abs(diff.y) > abs(diff.x):
						dir = Vector2i(0, sign(diff.y)) # push vertically
					else:
						dir = Vector2i(sign(diff.x), sign(diff.y)) # diagonal if equal

					rects[i].position += dir
					rects[j].position -= dir
		queue_redraw()
...
#rounds a number to move away from something
func round_away(x: float, away_target: float) -> int:
	if x == int(x):
		return int(x)
	if x > 0:
		if x > away_target:
...
func round_toward(x: float, toward_target: float) -> int:
	if x < toward_target:
		return round_away(x, -INF)
	else:
		return round_away(x, INF)
...
class SpatialGraph:
	...
	func _init(rects: Array[Rect2i], _params: DungeonParameters) -> void:
		for rect in rects:
			nodes.append(SpatialGraphNode.new(rect))
		params = _params
		generate_graph()
	...
	func generate_graph() -> void:
		...
		spanning_graph = PhysicsDungeonGenerator.create_spanning_graph(connections, false)
		generate_neighbors()
	...
	func add_extra_edges(percent: float) -> void:
		var unused_connections:Array[SpatialConnection] = []
